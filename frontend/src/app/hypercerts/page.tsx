          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <h2 className="font-bebas text-2xl uppercase tracking-widest text-white/80">Transaction History</h2>
                <div className="h-px flex-1 bg-white/10"></div>
              </div>

              {userRequests.length === 0 ? (
                <div className="py-20 text-center border border-dashed border-white/10 rounded-3xl bg-white/[0.01]">
                  <div className="text-white/20 font-bebas text-3xl uppercase tracking-widest">No Requests Found</div>
                  <p className="text-white/30 text-xs font-sans mt-2 tracking-wide">Your past minting requests will appear here once submitted.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {userRequests.map((request) => (
                    <div key={request.id} className="p-6 rounded-2xl border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-all group">
                      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            <span className="font-bebas text-xl uppercase text-white/90">Request #{request.id.slice(0, 8)}</span>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-mono uppercase tracking-tighter ${request.status === 'PENDING' ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' :
                              request.status === 'APPROVED' ? 'bg-brand-green/10 text-brand-green border border-brand-green/20' :
                                'bg-red-500/10 text-red-500 border border-red-500/20'
                              }`}>
                              {request.status}
                            </span>
                          </div>
                          <div className="flex items-center gap-4 text-[11px] text-white/30 font-mono tracking-wider">
                            <span>TIME: {new Date(request.submittedAt).toLocaleDateString()}</span>
                            {request.reviewedAt && <span>REVIEWED: {new Date(request.reviewedAt).toLocaleDateString()}</span>}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                          {request.hypercertId ? (
                            <div className="flex flex-col md:items-end gap-2">
                              <div className="flex items-center gap-2 px-4 py-2 bg-brand-green/5 border border-brand-green/20 rounded-xl">
                                <span className="text-brand-green text-[10px] font-bold uppercase tracking-widest">✅ ON-CHAIN</span>
                                <span className="text-xs font-mono text-white/50">{request.hypercertId}</span>
                              </div>
                              <div className="flex gap-4">
                                {request.txHash && (
                                  <a href={`https://celo-sepolia.blockscout.com/tx/${request.txHash}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase font-bold text-brand-yellow hover:text-white transition-colors tracking-widest underline decoration-brand-yellow/30 underline-offset-4">Explorers</a>
                                )}
                                {request.metadataCid && (
                                  <a href={`https://gateway.pinata.cloud/ipfs/${request.metadataCid}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase font-bold text-brand-yellow hover:text-white transition-colors tracking-widest underline decoration-brand-yellow/30 underline-offset-4">Metadata</a>
                                )}
                              </div>
                            </div>
                            ...
                            <button
                              ...
                              className="w-full md:w-auto px-8 py-3 bg-brand-green text-black font-bebas text-lg uppercase tracking-widest rounded-full hover:bg-white transition-all active:scale-95"
                            >
                              Mint Now
                            </button>
                          ) : request.status === 'REJECTED' ? (
                            <div className="px-4 py-2 bg-red-500/5 border border-red-500/20 rounded-xl text-[10px] text-red-500 max-w-xs uppercase tracking-wider leading-tight">
                              REASON: {request.rejectionReason}
                            </div>
                          ) : (
                            <div className="px-8 py-3 border border-white/10 text-white/20 font-bebas text-lg uppercase tracking-widest rounded-full cursor-default">
                              In Review
                            </div>
                          )}
                        </div>
