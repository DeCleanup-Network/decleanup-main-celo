  ...
  if (eventFromQuery) {
    return (
      <div className="mx-auto w-full max-w-md px-4 py-6">
        <p className={campaignText.note}>Opening cleanup…</p>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-5 px-4 py-6 sm:px-5">
      ...
      <section className="space-y-3">
        <h2 className={campaignText.label}>Open cleanups</h2>
        {loadingEvents ? (
          ...
          <div className="rounded-xl border border-white/10 bg-zinc-950/80 px-3 py-4">
            <p className={campaignText.note}>No active or upcoming cleanups yet.</p>
          </div>
        ) : (
                  ...
                  <article className="rounded-xl border border-white/10 bg-zinc-950/80 p-3 transition hover:border-white/20">
                    <Link href={href} className="block text-left">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`shrink-0 rounded px-1.5 py-0.5 uppercase tracking-wide ring-1 ring-white/10 ${campaignText.meta}`}>
                          {ev.status}
                        </span>
                      </div>
                      <p className={`mt-1 ${campaignText.note}`}>
                      ...
                      <p className={`mt-0.5 ${campaignText.meta}`}>{formatEventDate(ev.eventDate)}</p>
                      ...
                      <p className={`mt-1.5 ${campaignText.meta}`}>
                        <span className="text-zinc-100">{formatCusd(ev.amountRaisedCusd)}</span>
                        <span> / {formatCusd(ev.fundingGoalCusd)} cUSD · {verifiedCleanupLabel(ev.verifiedCleanupsCount)}</span>
                      </p>
                      {ev.whyFunding ? (
                        <p className={`mt-2 line-clamp-2 ${campaignText.body}`}>{ev.whyFunding}</p>
                      ) : null}
                    </Link>
                    <div className="mt-3">
